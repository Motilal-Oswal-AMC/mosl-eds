...
export default function decorate(block) {
  ...
  if (container) {
    ...
    const performanceWrapper = container.querySelector('.performance-graph-returning-wrapper');

    if (defaultContent && performanceWrapper && !defaultContent.parentElement.classList.contains('performance-graph-wrapper')) {
  ...
  // ---------- FUND HEADER ----------
  const fundHeader = div({ class: 'fund-header' },
    div({ class: 'fund-details' },
      ...
      div({ class: 'fund-meta' },
        div({ class: 'fund-tags-wrapper' },
          div({ class: 'fund-tags-main' },
            span({ class: 'fund-tag' }, 'Indian Equity')),
          div({ class: 'fund-tags-main' },
            span({ class: 'fund-tag' }, 'Large and Mid Cap')),
        ),
        div({ class: 'fund-returns-main' },
  ...
  const mobileInvestButton = div({ class: 'invest-now-btn-mobile' }, a({ href: ctaLink, class: 'button button-mobile' }, ctaText));
  ...
  function filterChartData(data, filter) {
    ...
    const filtered = sortedData.filter((item) => new Date(item.navdate) >= cutoffDate);
