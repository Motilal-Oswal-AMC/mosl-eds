...
export default function decorate(block) {
  ...
  Array.from(block.querySelector('.return-container .radio-label-container').children).forEach((elsearch) => {
    elsearch.querySelector('input').addEventListener('click', (event) => {
      const dataattr = event.target.getAttribute('dataattr').split('-');
      const tempdata = dataCfObj.filter((eltemp) => (dataattr.includes(eltemp.schcode) ? eltemp : ''));
      dataMapMoObj.tempMobReturn = [];
      dataMapMoObj.tempMobReturn = tempdata;
