...
const Swiper = (function () {
  ...
  const _ = {
    ...
    loopFix(e) {
      ...
      const { centeredSlides: f, initialSlide: v } = h;
      if (
        ((d.allowSlidePrev = !0),
        (d.allowSlideNext = !0),
      ...
      c.length < w + y || (d.params.effect === 'cards' && c.length < w + 2 * y)
        ? g(
      ...
      const x = [];
      const S = [];
      const T = E ? Math.ceil(c.length / h.grid.rows) : c.length;
      const M = n && T - v < w && !f;
  ...
  function V(e) {
    ...
    if (i.type === 'touchstart') return void q(t, i, i.targetTouches[0].pageX);
    ...
    breakpoints: {
      setBreakpoint() {
        ...
        ['navigation', 'pagination', 'scrollbar'].forEach((t) => {
          ...
          const a = u[t] && u[t].enabled;
          s && !a && e[t].disable(), !s && a && e[t].enable();
        });
        const w = u.direction && u.direction !== i.direction;
        const b = i.loop && (u.slidesPerView !== i.slidesPerView || w);
        const y = i.loop;
  ...
  const ve = [
      ...
      function d(e, t) {
        ...
        return (
          a.renderSlide
            ...
            typeof i === 'string' && (C(o, i), (i = o.children[0])))
            : (i = s.isElement
              ? v('swiper-slide')
    ...
    function (e) {
      ...
      function M() {
        ...
        const s = v.imageWrapEl.getAttribute('data-swiper-zoom') || e.maxRatio;
      ...
      function L(e) {
        if ((e.pointerType === 'mouse' && g.splice(0, g.length), !C(e))) return;
      ...
      function I(e) {
        ...
        const s = t.params.zoom;
        const a = t.zoom;
        const i = g.findIndex((t) => t.pointerId === e.pointerId);
        i >= 0 && (g[i] = e),
        g.length < 2
            || ((h = !0),
            (v.scaleMove = T()),
      ...
      function $(e) {
        const s = e.pointerType === 'mouse' && t.params.zoom.panOnMouseMove;
        if (!C(e) || !P(e)) return;
        const a = t.zoom;
        if (!v.imageEl) return;
        ...
        const { originX: c, originY: p } = v;
        (b.currentX = b.touchesCurrent.x
          - b.touchesStart.x
          + b.startX
          + n * (b.width - 2 * c)),
        (b.currentY = b.touchesCurrent.y
            - b.touchesStart.y
            + b.startY
            + n * (b.height - 2 * p)),
        b.currentX < b.minX
        ...
        (y.x = (b.touchesCurrent.x - y.prevPositionX)
            / (Date.now() - y.prevTime)
            / 2),
        (y.y = (b.touchesCurrent.y - y.prevPositionY)
            / (Date.now() - y.prevTime)
            / 2),
      ...
      function O(e) {
        ...
        const i = (e.clientY - p.y) * u;
        const r = b.width * l;
        const o = b.height * l;
        const d = v.slideWidth;
        const m = v.slideHeight;
        const h = Math.min(d / 2 - r / 2, 0);
        const f = -h;
        const g = Math.min(m / 2 - o / 2, 0);
        const w = -g;
        const y = Math.max(Math.min(b.startX + a, f), h);
        const E = Math.max(Math.min(b.startY + i, w), g);
        (v.imageWrapEl.style.transitionDuration = '0ms'),
      ...
      function D(e) {
        ...
        let i; let r; let o; let d; let c; let p; let u; let m; let h; let g; let y; let x; let S; let T; let C; let P; let L; let
          I;
        t.params.cssMode
          && ((t.wrapperEl.style.overflow = 'hidden'),
        ...
        !e || (l === 1 && A)
          ...
          z > 0
              && A
              && typeof b.currentX === 'number'
              && typeof b.currentY === 'number'
          ...
          m < T && (m = T),
          m > P && (m = P)),
        A && s.scale === 1 && ((v.originX = 0), (v.originY = 0)),
        (b.currentX = u),
        (b.currentY = m),
      ...
      a('touchEnd', (e, s) => {
        t.zoom.enabled
            && (function () {
              ...
              let a = 300;
              const i = y.x * s;
              const r = b.currentX + i;
              const n = y.y * a;
              const l = b.currentY + n;
              y.x !== 0 && (s = Math.abs((r - b.currentX) / y.x)),
              y.y !== 0 && (a = Math.abs((l - b.currentY) / y.y));
